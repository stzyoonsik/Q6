package gameScene.ui
{
	...
	public class IngameUi extends GameObject
	{
		...
		private const SETTING:int = 3;
		private const FLAG:int = 4;
		// + Setting Popup
		...
		public function setCurrentFlag(numFlag:int):void
		{
			...
			for (var i:int = totalFlag.length - 1; i >= 0; i--)
			{
				...
				var digit:GameObject = flag.getChild(i);	
				
				if (digit)
				{
					var image:Image = digit.components[ComponentType.IMAGE];
					
					if (image)
					{
						image.texture = _numbers[currentFlag.charAt(currentFlagIndex)];
					}
				}
				currentFlagIndex--;
			}
		...
		private function onCompleteLoadAll():void
		{
			...
			// SETTING
			var setting:Button = new Button(new Texture(_bitmaps["setting"]));
			
			setting.x = MainStage.stageWidth - setting.width;
			setting.y = fieldsXY * 2.5;
			...
			for (i = 0; i < numFlag.length * 2 + 1; i++)
			{
				...
				if (i < numFlag.length)
				{
					...
					digit.addComponent(new Image(_numbers["0"]));
				}
